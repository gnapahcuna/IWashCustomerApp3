...
class _LoginPageState extends State<LoginPage>
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: MaterialButton(
              ...
              onPressed: () =>
                  _onSignIn(loginTelephoneController.text,
          ...
          Container(
            ...
            child: MaterialButton(
                ...
                onPressed: () {
                  ...
                  firstname.isEmpty
                      ? _validateFirstnameSignUp = true
                      : _validateFirstnameSignUp = false;
                  lastname.isEmpty
                      ? _validateLastnameSignUp = true
                      : _validateLastnameSignUp = false;
                  telephone.isEmpty
                      ? _validatePhoneSignUp = true
                      : _validatePhoneSignUp = false;
                  password.isEmpty
                      ? _validatePasswordSignUp = true
                      : _validatePasswordSignUp = false;

                  if (_validateFirstnameSignUp == false &&
                      _validateLastnameSignUp == false &&
                      _validatePhoneSignUp == false &&
                      _validatePasswordSignUp == false) {
                    _onSignUp(
  ...
  _putUserAcct(CustomerID, BranchID, BranchGroupID, BranchCode, Firstname,
      Lastname, phone, BranchNameTH, BranchNameEN, TelephoneBranch) async {
      ...
      callSignInUser(telephone, password, ipAddress).then((s) async {
        ...
        isData != true
          ...
          _putUserAcct(
              users[0].CustomerID,
              users[0].BranchID,
              users[0].BranchGroupID,
              users[0].BranchCode
              ,
              users[0].FirstName,
              users[0].LastName,
              users[0].TelephoneNo,
              users[0].BranchNameTH,
              users[0].BranchNameEN,
              users[0].TelephoneNoBranch);
        }) : _showDialog(context, "เตือน!", "Login ไม่ถูกต้อง.", "success");
      });
    }
  }
  ...
  void _onSignUp(firstname, lastname, telephone, password,
    ...
    if (firstname
      ...
      resp.then((s) async {
        ...
        if (s.body == 'Repeat') {
          _showDialog(
              context, "เตือน!", "หมายเลขมือถือนี้มีในระบบแล้ว", "success");
        } else {
          Navigator.of(context).push(
            new MaterialPageRoute(
              builder: (context) {
                ...
                return new VerifyOTP(
                  Telephone: telephone, PageIndex: 'sign-up',);
              },
            ),
          );
        }
          ...
          actions: <Widget>[
            ...
            new CupertinoButton(
                onPressed: () {
                  ...
                  print("type : " + type);
                  var phone = loginTelephoneController.text.toString();
                  Navigator.pop(context);
                  if (type == "error") {
                    //Navigator.pop(context);
                    final data = callChangeOTP(phone, code, mContext);
                    data.then((s) async {
                      showDialog(
                          context: context,
                          builder: (BuildContext context) {
                      ...
                      if (s.body.trim() == "Non-data") {
                        _showDialog(mContext, "เตือน!",
                            "หมายเลขโทรศัพท์นี้ไม่มีในระบบ",'success');
                      } else {
                        if (s.body != null) {
                            ...
                            setState(() {
                              Navigator.of(mContext).push(
                                new MaterialPageRoute(
                                  builder: (context) {
                                    //return new SearchList();
                                    return new VerifyOTP(Telephone: phone,PageIndex: 'sign-up',);
                                  },
                                ),
                              );
                            });
                  ...
                  /*if(type=="error"){
                    //print(loginTelephoneController.text.toString());
                    Navigator.of(mContext).push(
                      new MaterialPageRoute(
                        builder: (context) {
                          //return new SearchList();
                ...
                child: new Text(type=="error"?'Verify':'ตกลง', style: TextStyle(color: Colors.red,
                    fontSize: 16.0,
                    fontWeight: FontWeight.bold,
                    fontFamily: 'Poppins'))),
          ]
  ...
  void _showDialog(mContext, title, desc, type) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return _createCupertinoAlertDialog(mContext, title, desc, type);
      },
    );
  }


  Future<List<_users.Users>> callSignInUser(telephone, password,
      ipAddress) async {
    final paramDic = {
      "Telephone": telephone,
      "Password": password,
    ...
    final response = await http.post(
      Server().IPAddress + "/put/customer/UserSignIn.php",
      // change with your API
      body: paramDic,
    );
    if (response.statusCode == 200) {
      print(response.body);
      if (response.body == "NotVerify") {
        _showDialog(context, "เตือน!", "เบอร์มือถือนี้ยังไม่ Verify", "error");
      }
      else if (response.body == "NotUser") {
        _showDialog(context, "เตือน!", "Login ไม่ถูกต้อง.", "success");
      } 
      else if(response.body.contains("NotBranch")){
        List splits = response.body.split(",");
        String custID = splits[1];
        print(custID);
        Future.delayed(const Duration(milliseconds: 500), () {
          setState(() {
            Navigator.of(context).push(
              new MaterialPageRoute(
                builder: (context) {
                  //return new SearchList();
                  return new SelectBranch(CustomerID: custID,IsPage: "OTP",);
                },
              ),
            );
          });
...
Future<http.Response> callChangeOTP(tel,otp,mContext) async {
  final paramDic = {
    "Telephone": tel,
    "OTP": otp,
  ...
  final loginData = await http.post(
    Server().IPAddress + "/put/customer/VerifyAgain.php", // change with your API
    body: paramDic,
  );
